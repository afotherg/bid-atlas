...
test("Monrovia publishes its current Old Town business improvement district", () => {
  const districts = collection.features.filter((feature) => feature.properties.sourceId === "monrovia-old-town-business-improvement-district");
  assert.equal(districts.length, 1);
  const [district] = districts;
  assert.equal(district.properties.established, "1968");
  assert.equal(district.properties.status, "Active");
  const [longitude, latitude] = district.properties.center;
  assert.ok(longitude > -119 && longitude < -117);
  assert.ok(latitude > 33 && latitude < 35);
...
test("Glendale publishes its two active business and community benefit districts", () => {
  const districts = collection.features.filter((feature) => feature.properties.sourceId === "glendale-business-improvement-districts");
  const expected = new Map([
    ["Greater Downtown Glendale Community Benefit District", ["2018", "2038"]],
    ["Montrose Business Improvement District", ["Before 2004", null]],
  ]);
  assert.equal(districts.length, 2);
  for (const district of districts) {
    const term = expected.get(district.properties.name);
    assert.ok(term, `unexpected Glendale district ${district.properties.name}`);
    assert.equal(district.properties.established, term[0]);
    assert.equal(district.properties.expires ?? null, term[1]);
    assert.equal(district.properties.status, "Active");
    const [longitude, latitude] = district.properties.center;
    assert.ok(longitude > -119 && longitude < -117);
    assert.ok(latitude > 33 && latitude < 35);
...
test("Austin publishes its three maintenance and operations public improvement districts", () => {
  const districts = collection.features.filter((feature) => feature.properties.sourceId === "austin-maintenance-operations-public-improvement-districts");
  const names = new Set(districts.map((feature) => feature.properties.name));
  assert.equal(districts.length, 3);
  for (const name of [
    "Austin Downtown Public Improvement District",
    "East 6th Street Public Improvement District",
    "South Congress Preservation and Improvement District",
  ]) assert.ok(names.has(name), `missing ${name}`);
  assert.ok(!names.has("Austin Tourism Public Improvement District"));
  for (const district of districts) {
    assert.equal(district.properties.city, "Austin");
    assert.equal(district.properties.state, "TX");
    assert.equal(district.properties.status, "Active");
    assert.match(district.properties.area, /Generalized boundary/);
    const [longitude, latitude] = district.properties.center;
    assert.ok(longitude > -98 && longitude < -97.5, `${district.properties.name} is outside Austin longitude`);
    assert.ok(latitude > 30.1 && latitude < 30.4, `${district.properties.name} is outside Austin latitude`);
  }
});
...
test("Oklahoma City publishes its six active business improvement districts", () => {
  const districts = collection.features.filter((feature) => feature.properties.sourceId === "oklahoma-city-business-improvement-districts");
  const names = new Set(districts.map((feature) => feature.properties.name));
  assert.equal(districts.length, 6);
  for (const name of [
    "Downtown Oklahoma City Business Improvement District",
    "OKC Adventure District Business Improvement District",
    "Stockyards City Business Improvement District",
    "Capitol Hill Business Improvement District",
    "Uptown 23rd Business Improvement District",
    "Western Avenue Business Improvement District",
  ]) assert.ok(names.has(name), `missing ${name}`);
  for (const district of districts) {
    assert.equal(district.properties.city, "Oklahoma City");
    assert.equal(district.properties.state, "OK");
    assert.equal(district.properties.status, "Active");
    assert.match(district.properties.area, /Generalized outer service area/);
    const [longitude, latitude] = district.properties.center;
    assert.ok(longitude > -97.7 && longitude < -97.3, `${district.properties.name} is outside Oklahoma City longitude`);
    assert.ok(latitude > 35.3 && latitude < 35.7, `${district.properties.name} is outside Oklahoma City latitude`);
  }
});
...
test("Detroit publishes its three active business improvement zones", () => {
  const sourceIds = new Set([
    "detroit-downtown-business-improvement-zone",
    "detroit-neighborhood-business-improvement-zones",
  ]);
  const districts = collection.features.filter((feature) => sourceIds.has(feature.properties.sourceId));
  const names = new Set(districts.map((feature) => feature.properties.name));
  assert.equal(districts.length, 3);
  for (const name of [
    ...
    "Rivertown Business Improvement Zone",
    "West Vernor & Springwells Business Improvement District",
  ]) assert.ok(names.has(name), `missing ${name}`);
  assert.equal(districts.find((feature) => feature.properties.name === "Rivertown Business Improvement Zone")?.properties.expires, "2035");
  for (const district of districts) {
    assert.equal(district.properties.city, "Detroit");
    assert.equal(district.properties.state, "MI");
    assert.equal(district.properties.status, "Active");
    const [longitude, latitude] = district.properties.center;
    assert.ok(longitude > -83.2 && longitude < -82.9, `${district.properties.name} is outside Detroit longitude`);
    assert.ok(latitude > 42.2 && latitude < 42.5, `${district.properties.name} is outside Detroit latitude`);
  }
});
