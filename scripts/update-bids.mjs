...
function normalize(source, feature) {
  ...
  const properties = {
    ...
    city,
    state: source.state,
    area: override.area ?? valueFor(p, source.fields.area),
    website: override.website ?? valueFor(p, source.fields.website),
    established: override.established ?? valueFor(p, source.fields.established),
    expires: override.expires ?? valueFor(p, source.fields.expires),
    annualRevenue: override.annualRevenue ?? valueFor(p, source.fields.annualRevenue),
    reportUrl: override.reportUrl ?? valueFor(p, source.fields.reportUrl),
    status: override.status ?? valueFor(p, source.fields.status) ?? "Active",
    sourceId: source.id,
